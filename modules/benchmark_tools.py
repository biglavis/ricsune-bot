...
class NumbersView(discord.ui.View):
    def __init__(self, ctx: commands.Context, timeout: int = 180):
        super().__init__(timeout=timeout)
        self.ctx = ctx
        self.controller: Numbers = None
        self.message: discord.Message = None
...
class Numbers():
    def __init__(self, ctx: commands.Context):
        self.ctx = ctx
        self.view: NumbersView = None
        self.level = 0

    async def start(self):
        embed = discord.Embed(title="Number Memory", description="The average person can remember 7 numbers at once.\nCan you do more?")
    ...
    async def scramble(self, interaction: discord.Interaction):
        number = random.randint(pow(10, self.level-1), pow(10, self.level)-1)
        time = datetime.datetime.now(datetime.timezone.utc).astimezone()

        self.view.remove_all()
        self.view.add_item(discord.ui.TextInput(label="test"))
        embed = discord.Embed(title=f"{number}", description=f"<t:{time.timestamp()}:R>")
    ...
    async def interacted(self, button: Button, interaction: discord.Interaction):
        ...
        if self.level == 0:
            self.level +=1
            await self.scramble(interaction=interaction)
    ...
    async def completed(self):
        ...
        if 'numbers' not in leaderboard:
            leaderboard['numbers'] = {}

        # if user not in leaderboard or user has new highscore
        if str(self.ctx.author.id) not in leaderboard['numbers'] or (score > 0 and score > leaderboard['numbers'][str(self.ctx.author.id)]):
            leaderboard['numbers'][str(self.ctx.author.id)] = score

        # sort leaderboard
        leaderboard['numbers'] = dict(sorted(leaderboard['numbers'].items(), key=lambda item: item[1], reverse=True))
        if len(leaderboard['numbers']) > 5:
            leaderboard['numbers'] = leaderboard['numbers'][:5]
        ...
        description="**Leaderboard**"
        for id in leaderboard['numbers']:
            description += f"\n**{leaderboard['numbers'][id]}**" + " \u200b"*3 + "\u00B7" + " \u200b"*3 + f"<@{id}>"
